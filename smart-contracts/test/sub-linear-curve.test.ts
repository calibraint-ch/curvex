...
import mock from "./mock/sub-linear.mock";
import { sqrt } from "./service";

const AddZero = ethers.constants.AddressZero;

describe("Sub-linear - Token Manager - BondingCurve.sol", function () {
  after(async () => {
  ...
  it("should be able to mint token from token manager", async () => {
    ...
    const sq = sqrt(buyAmount.mul(DECIMALS));
    const spendAmount = sq
      .mul(sq)
      .mul(sq)
      .mul(DECIMALS)
      .mul(3)
    ...
    console.log({ spendAmount, buyAmount });
  ...
  it("should be able sell tokens immediately after selling", async () => {
    ...
    const tokenToSell2 = tokenToSell;

    const sellTotalSupplyPoolBalance = tokenToSell
      .mul(sqrt(tokenToSell))
      .mul(DECIMALS)
      .mul(3)
      .div(2)
      .div(PRECISION)
    ...
    console.log("sent", tokenToSell);
    console.log("expected", sell1Return);
    await expect(tokenManager.connect(addr1).sell(tokenToSell))
      .to.emit(tokenB, "Transfer")
